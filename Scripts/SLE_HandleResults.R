...
res_files <- list.files(path="./output_data/SAIGE/SLE/Results/", pattern="SAIGE_SBA_chr*")
...
res_ALL001 <- fread(file="./output_data/SAIGE/SLE/Results/SAIGE_SVA_sparseGRM_Firth_SLE_RESULTS_maf001.txt", header=T)
res_ALL005 <- fread(file="./output_data/SAIGE/SLE/Results/SAIGE_SVA_sparseGRM_Firth_SLE_RESULTS_maf005.txt", header=T)
#----------------------------------
#         QQ Plot Function
#----------------------------------
...
#----------------------------------------------------------
#----------------------------------------------------------
res_ALL001_chr6 <- subset(res_ALL001, subset=(CHR == 6))
res_ALL001_chr6_reg <- subset(res_ALL001_chr6, subset=((POS >= 143863601)&(POS <=173863601)))
hist(res_ALL001_chr6_reg$p.value)
sum(res_ALL001_chr6_reg$p.value < 5e-08)
#[1] 0
res_ALL001_chr6_reg$p.value_log <- -log10(as.numeric(res_ALL001_chr6_reg$p.value))
res_ALL001_chr6_reg$SNP_ID <- paste(res_ALL001_chr6_reg$MarkerID,res_ALL001_chr6_reg$Allele1,res_ALL001_chr6_reg$Allele2,sep=":")

res_ALL001_chr5 <- subset(res_ALL001, subset=(CHR == 5))
res_ALL001_chr5_reg <- subset(res_ALL001_chr5, subset=((POS >= 140749194)&(POS <=188279842)))
hist(res_ALL001_chr5_reg$p.value)
sum(res_ALL001_chr5_reg$p.value < 5e-08)
#[1] 0
res_ALL001_chr5_reg$p.value_log <- -log10(as.numeric(res_ALL001_chr5_reg$p.value))
res_ALL001_chr5_reg$SNP_ID <- paste(res_ALL001_chr5_reg$MarkerID,res_ALL001_chr5_reg$Allele1,res_ALL001_chr5_reg$Allele2,sep=":")
#--------------------------
res_ALL005_chr6 <- subset(res_ALL005, subset=(CHR == 6))
res_ALL005_chr6_reg <- subset(res_ALL005_chr6, subset=((POS >= 143863601)&(POS <=173863601)))
hist(res_ALL005_chr6_reg$p.value)
sum(res_ALL005_chr6_reg$p.value < 5e-08)
#[1] 0
res_ALL005_chr6_reg$p.value_log <- -log10(as.numeric(res_ALL005_chr6_reg$p.value))
res_ALL005_chr6_reg$SNP_ID <- paste(res_ALL005_chr6_reg$MarkerID,res_ALL005_chr6_reg$Allele1,res_ALL005_chr6_reg$Allele2,sep=":")

res_ALL005_chr5 <- subset(res_ALL005, subset=(CHR == 5))
res_ALL005_chr5_reg <- subset(res_ALL005_chr5, subset=((POS >= 140749194)&(POS <=188279842)))
hist(res_ALL005_chr5_reg$p.value)
sum(res_ALL005_chr5_reg$p.value < 5e-08)
#[1] 0
res_ALL005_chr5_reg$p.value_log <- -log10(as.numeric(res_ALL005_chr5_reg$p.value))
res_ALL005_chr5_reg$SNP_ID <- paste(res_ALL005_chr5_reg$MarkerID,res_ALL005_chr5_reg$Allele1,res_ALL005_chr5_reg$Allele2,sep=":")
#---------------------------------------------------------------
#                       Manhattan Plots
#---------------------------------------------------------------
#---------------------------------------
...
sig <- (nrow(res_ALL001) * 5e-08) / nrow(res_ALL001_chr6_reg)  # 4.4e-06  -log10(sig) = 5.4

ggplot(res_ALL001_chr6_reg, aes(x = POS, y = -log10(p.value), color = as.factor(CHR), size = -log10(p.value), label=SNP_ID)) +
   ...
   geom_label_repel(data = subset(res_ALL001_chr6_reg, subset=(-log10(p.value) >= 4)), size = 3, max.overlaps = 20, box.padding = 0.01, point.padding = 0.01, segment.color = 'grey50') +
   ...
   theme_minimal() +
   labs(title="SAIGE SVA: SLE Associations",
        x ="",
        y ="-log<sub>10</sub>(p)") +
...
sig <- (nrow(res_ALL005) * 5e-08) / nrow(res_ALL005_chr6_reg)  # 4.2e-06
ggplot(res_ALL005_chr6_reg, aes(x = POS, y = -log10(p.value), color = as.factor(CHR), size = -log10(p.value), label=SNP_ID)) +
   geom_hline(yintercept = c(-log10(sig), -log10(5e-08)), color = c("skyblue4", "hotpink4"), linetype = c("dashed", "solid"), lwd = 1) +
   geom_point(alpha = 0.70) +
   geom_label_repel(data = subset(res_ALL005_chr6_reg, subset=(-log10(p.value) >= 4)), size = 3, max.overlaps = 20, box.padding = 0.01, point.padding = 0.01, segment.color = 'grey50') +
   scale_color_manual(values = rep(c("midnightblue", "midnightblue"))) +
   scale_size_continuous(range = c(0.5, 3)) +
   theme_minimal() +
   labs(title="SAIGE SVA: SLE Associations", x ="Chromosome 6 position", y ="-log<sub>10</sub>(p)") +
   theme(title = element_text( size = 14), axis.text = element_text(size = 12), axis.title = element_text(size = 14),
         legend.position = "none",
         panel.grid.major.x = element_blank(),
         panel.grid.minor.x = element_blank(),
...
#---------------------------------------
## Suggestive significance level for the proportion of the genome tested:
sig <- (nrow(res_ALL001) * 5e-08) / nrow(res_ALL001_chr5_reg)  # 3.4e-06  -log10(sig) = 5.5

ggplot(res_ALL001_chr5_reg, aes(x = POS, y = -log10(p.value), color = as.factor(CHR), size = -log10(p.value), label=SNP_ID)) +
  geom_hline(yintercept = c(-log10(sig), -log10(5e-08)), color = c("skyblue4", "hotpink4"), linetype = c("dashed", "solid"), lwd = 1) +
  geom_point(alpha = 0.70) +
  geom_label_repel(data = subset(res_ALL001_chr5_reg, subset=(-log10(p.value) >= 4.2)), size = 3, max.overlaps = 20, box.padding = 0.01, point.padding = 0.01, segment.color = 'grey50') +
  scale_color_manual(values = rep(c("midnightblue", "midnightblue"))) +
  scale_size_continuous(range = c(0.5, 3)) +
  scale_x_continuous(breaks=seq(92000000,120000000,by=5000000), labels=scales::comma) +
  #scale_x_continuous(breaks=NULL) +
...
sig <- (nrow(res_ALL005) * 5e-08) / nrow(res_ALL005_chr5_reg)  # 3.3e-06
ggplot(res_ALL005_chr5_reg, aes(x = POS, y = -log10(p.value), color = as.factor(CHR), size = -log10(p.value), label=SNP_ID)) +
   geom_hline(yintercept = c(-log10(sig), -log10(5e-08)), color = c("skyblue4", "hotpink4"), linetype = c("dashed", "solid"), lwd = 1) +
   geom_point(alpha = 0.70) +
   geom_label_repel(data = subset(res_ALL005_chr5_reg, subset=(-log10(p.value) >= 4)), size = 3, max.overlaps = 20, box.padding = 0.01, point.padding = 0.01, segment.color = 'grey50') +
   scale_color_manual(values = rep(c("midnightblue", "midnightblue"))) +
   scale_size_continuous(range = c(0.5, 3)) +
   theme_minimal() +
   labs(title="SAIGE SVA: SLE Associations", x ="Chromosome 5 position", y ="-log<sub>10</sub>(p)") +
   theme(title = element_text( size = 14), axis.text = element_text(size = 12), axis.title = element_text(size = 14),
         legend.position = "none",
         panel.grid.major.x = element_blank(),
         panel.grid.minor.x = element_blank(),
...
length(unique(res_GB$SAIGE_SBA_chr6$Region))
# 908 GENES on Chromosome 6
length(unique(res_GB$SAIGE_SBA_chr5$Region))
# 745 GENES on Chromosome 5
...
res_chr6 <- res_GB$SAIGE_SBA_chr6
...
res_chr5 <- res_GB$SAIGE_SBA_chr5
...
# 861 ACAT2 160183510 160200144     3 missense;lof;synonymous     0.5     NA 1- line, 3 synonymous variants
subset(res_chr6, subset=(Region == "ACAT2"))
subset(gf_chr6, subset=(V1 == "ACAT2"))
...
GENES_chr5[ind,1:7]
#       Gene     Start       End N_Var                   Group max_MAF Pvalue
# 77  TTC23L  34839268  34900606     4                    <NA>      NA     NA - 4 variants; 2 missense, 2 synonymous
# 363  ISOC1 128430441 128449719     2 missense;lof;synonymous     0.5     NA - 1 line, 2 synonymous variants
# 613  THG1L 157158427 157168457     2 missense;lof;synonymous     0.5     NA - 1 line, 2 synonymous variants
subset(res_chr5, subset=(Region == "THG1L"))
subset(gf_chr5, subset=(V1 == "THG1L"))
## Remove genes with NA pvalues
GENES_chr5 <- subset(GENES_chr5, subset=(!is.na(Pvalue)))

#----------------------------------------------------------------------------------
#----------------------------------------------------------------------------------
# WE HAVE 908 GENES ON CHR6
...
# [1] 6 "Mito" genes in chr5 region
#------------------------------------------------------------------------------------
ggplot(GENES_chr6_reg,aes(x=as.numeric(Start),y=-log10(as.numeric(Pvalue)), color = Mito, label=Gene)) + geom_point() +
  geom_hline(yintercept=c(-log10(0.05/10),-log10(0.05/114)), color = c("orange", "midnightblue"), linetype = c("dashed", "solid"), lwd = 1) +
...
#            Chromosome 6
#--------------------------------------
sig <- (nrow(res_ALL001) * 5e-08) / nrow(res_ALL001_chr6_reg)  # 4.4e-06
MANHATTAN6 <- ggplot(res_ALL001_chr6_reg, aes(x = POS, y = -log10(p.value), color = as.factor(CHR), size = -log10(p.value))) +
...
min(res_ALL001_chr6_reg$p.value)
which(res_ALL001_chr6_reg$p.value == 4.816241e-06)
res_ALL001_chr6_reg[20757,]
# 6:151292437:G:A --- MTHFD1L gene
#--------------------------------------
#            Chromosome 5
#--------------------------------------
sig <- (nrow(res_ALL001) * 5e-08) / nrow(res_ALL001_chr5_reg)  # 3.4e-06
MANHATTAN5 <- ggplot(res_ALL001_chr5_reg, aes(x = POS, y = -log10(p.value), color = as.factor(CHR), size = -log10(p.value))) +
  geom_hline(yintercept = c(-log10(sig), -log10(5e-08)), color = c("gray","#fe7f2d"), linetype = c("dashed", "solid"), lwd = 1) +
  geom_point(alpha = 0.70) +
  scale_color_manual(values = rep(c("#233d4d", "#233d4d"))) +
  scale_size_continuous(range = c(0.5, 3)) +
...
min(res_ALL001_chr5_reg$p.value)
which(res_ALL001_chr5_reg$p.value == 5.152685e-05)
res_ALL001_chr5_reg[74572,]
# 5:168014668:C:G
res_ALL001_chr5_reg[74577,]
# 5:168019443:C:G 
#-------------------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------------------
...
res_ALL001_chr6_reg$GeneLabel <- "Other"
for(i in 1:nrow(res_ALL001_chr6_reg)){
  if(res_ALL001_chr6_reg$POS[i] %in% TULP4_var){res_ALL001_chr6_reg$GeneLabel[i] <- "TULP4"}
  else if(res_ALL001_chr6_reg$POS[i] %in% FRMD1_var){res_ALL001_chr6_reg$GeneLabel[i] <- "FRMD1"}
  else if(res_ALL001_chr6_reg$POS[i] %in% MTHFD1L_var){res_ALL001_chr6_reg$GeneLabel[i] <- "MTHFD1L"}}

# GWAS was done with MAF > 0.01 cut-off so some variants were removed
sum(res_ALL001_chr6_reg$GeneLabel=="TULP4")
#[1] 7
sum(res_ALL001_chr6_reg$GeneLabel=="FRMD1")
##[1] 10
sum(res_ALL001_chr6_reg$GeneLabel=="MTHFD1L")
##[1] 2

sig <- (nrow(res_ALL001) * 5e-08) / nrow(res_ALL001_chr6_reg)  # 4.4e-06
ggplot(data = res_ALL001_chr6_reg, aes(x = POS, y = -log10(p.value))) +
...
res_ALL001_chr5_reg$GeneLabel <- "Other"
for(i in 1:nrow(res_ALL001_chr5_reg)){
  if(res_ALL001_chr5_reg$POS[i] %in% SPINK5_var){res_ALL001_chr5_reg$GeneLabel[i] <- "SPINK5"}}

# GWAS was done with MAF > 0.01 cut-off so some variants were removed
sum(res_ALL001_chr5_reg$GeneLabel=="SPINK5")
#[1] 19

sig <- (nrow(res_ALL001) * 5e-08) / nrow(res_ALL001_chr5_reg)  # 3.4e-06
ggplot(data = res_ALL001_chr5_reg, aes(x = POS, y = -log10(p.value))) +
  geom_hline(yintercept =-log10(sig), color = "gray", linetype = "dashed", lwd = 1) +
  geom_point(aes(color = GeneLabel, alpha = GeneLabel)) +
